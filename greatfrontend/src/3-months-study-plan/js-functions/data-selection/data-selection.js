...
export default function selectData(sessions, options) {
  ...
  function mergeEquipments(eq1, eq2) {
    const eqSet = new Set();
    eq1.forEach((e) => eqSet.add(e));
    eq2.forEach((e) => eqSet.add(e));
    return Array.from(eqSet).sort();
  }

  const map = new Map();
  let mergedSessions = shouldMerge ? [] : sessions;
  if (shouldMerge) {
    for (let index = sessions.length - 1; index >= 0; index--) {
      const currSession = sessions[index];
      const { user, duration, equipment } = currSession;
      // if user is already present in map
      if (map.has(user)) {
        const existingSessionDetails = map.get(user);
        const combinedDuration = existingSessionDetails.duration + duration;
        const uniqueEquipments = mergeEquipments(
          existingSessionDetails.equipment,
          equipment
        );
        map.set(user, {
          ...existingSessionDetails,
          duration: combinedDuration,
          equipment: uniqueEquipments,
        });
      }
      // if user is not present, add current session to mergedSessions
      else {
        map.set(user, currSession);
      }
    }
    // build array of sessions to apply filters on
    mergedSessions = new Array(map.size);
    let mergedSessionIndex = mergedSessions.length - 1;
    map.entries().forEach(([_, currValue]) => {
      mergedSessions[mergedSessionIndex--] = currValue;
    });
  }
  ...
  const filteredSessions = mergedSessions.filter((session) => {
    ...
    if (
      ...
      !session.equipment.some((eq) => equipments.includes(eq))
    ) {
      return false;
    }
    return true;
