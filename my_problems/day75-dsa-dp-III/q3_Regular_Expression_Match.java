...
public class q3_Regular_Expression_Match {
    ...
    /**
     * Approach 2 - recursion + DP - using arrays not working
     * TC: O(n x m)
     * SC: recursion stack space O(max(n,m)) + dp array size O(n x m)
     */
    public int recursion_dp(int n, int m, String A, String B, int[][] dp) {
        ...
        if ((aChar == bChar) || (bChar == '?')) {
            // both characters match or ? is present in pattern
            state = recursion_dp(n - 1, m - 1, A, B, dp);
            dp[n][m] = state;
        } else if (bChar == '*') {
            // pick nothing for * || pick a character for *
            state = ((recursion_dp(n, m - 1, A, B, dp) == 1) || (recursion_dp(n - 1, m, A, B, dp) == 1)) ? 1 : 0;
            dp[n][m] = state;
        } else {
    ...
    public static void main(String[] args) {
        ...
        }
        {
            ...
            System.out.println(t1.recursion_dp(n - 1, m - 1, A, B, dp)); // true
