...
const compareSortedSubarrayBF = (A, B) => {
    ...
    for (let i = 0; i < B.length; i++) {
        ...
        for (let j = l1; j <= r1; j++) {
            ...
            if (hashMap.has(A[j])) {
                hashMap.set(A[j], hashMap.get(A[j]) + 1);
            } else {
                hashMap.set(A[j], 1);
            }
        }

        // for l2 to r2, remove from hashMap
        for (let j = l2; j <= r2; j++) {
            // remove elements in hashMap or update frequencies
            if (hashMap.has(A[j])) {
                const currentFreq = hashMap.get(A[j]);
                if (currentFreq === 1) {
                    // remove from map
                    hashMap.delete(A[j]);
                } else {
                    hashMap.set(A[j], currentFreq - 1);
                }
            } else {
                // break or update answer
                break;
            }
        }
