 ...
 * @param {Number} N no of discs
 * @param {Number} S Tower 1
 * @param {Number} T Tower 2
 * @param {Number} D Tower 3
 * @param {Number} output 2 D array with information of each disc movement
 * @returns 
 */
const towerOfHanoiRecursive = (N, S, T, D, output) => {
    if (N === 0) {
        return;
    }
    towerOfHanoiRecursive(N - 1, S, D, T, output);
    output.push([N, S, D]);
    towerOfHanoiRecursive(N - 1, T, S, D, output);
}
...
const towerOfHanoi = (N) => {
    let output = [];
    towerOfHanoiRecursive(N, 1, 2, 3, output);
    console.log(output);
}
